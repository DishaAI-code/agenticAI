...
def process_pdf_and_ask(uploaded_pdf, question, client):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp_file:
        tmp_file.write(uploaded_pdf.read())
        tmp_pdf_path = tmp_file.name
    ...
    loader = PyPDFLoader(tmp_pdf_path)
    documents = loader.load()
    return rag_answer_from_text(question, documents, client)

def rag_answer_from_text(question, documents, client):
    tokenizer = HuggingFaceEmbeddings(model_name=EMBEDDING_MODEL)
    splitter = RecursiveCharacterTextSplitter(
        chunk_size=512,
        chunk_overlap=50,
